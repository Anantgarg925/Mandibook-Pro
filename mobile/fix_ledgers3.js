...
async function fix() {
  ...
  const { data: buyers, error: buyErr } = await supabase.from('buyers').select('id, name, code, party_type, outstanding_balance, shop_id');
  ...
  // Create a map for quick party_type lookup
  const partyTypeMap = {};
  for (const b of buyers) {
    partyTypeMap[b.code] = b.party_type || 'BUYER';
  }
  ...
  for (const tx of (allTx || [])) {
    ...
    const pType = partyTypeMap[tx.buyer_code] || 'BUYER';
